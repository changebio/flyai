...
for step in range(dataset.get_step()):
    ...
    y_val = torch.from_numpy(y_val).to(device, dtype=torch.int64)
    ...
    correct = (prediction == torch.max(y_val, 1)[1]).sum().item()
    ...
    if accuracy > best_score:
        ...
        print("Step: {}, best accuracy is {}".format(step, best_score))
