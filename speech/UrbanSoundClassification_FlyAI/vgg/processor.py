...
def predict_data(fs):
    ...
    wav = Parallel(n_jobs=num_cores)(delayed(read_wav)(f,3) for f in fs)
...
class ImageLabel(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #i=np.random.randint(22050)
        y = self.wav[index]
        #print(ID,label)
        ps = librosa.feature.melspectrogram(y=y)
        #print(ID,label,ps.shape)
