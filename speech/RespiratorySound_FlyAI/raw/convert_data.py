...
def convert(from_dir, to_dir, file_out):
    names = ['URTI', 'Healthy', 'Asthma', 'COPD']
    unk_label = 'unknown'
    ...
    filename_set = set()
    rep_index = 0
    if not os.path.exists(os.path.join(to_dir, 'audio_and_txt_files')):
        os.makedirs(os.path.join(to_dir, 'audio_and_txt_files'))
    for root, dirs, filenames in os.walk(from_dir):
        ...
        for filename in filenames:
            if not filename.endswith('.wav'):
                continue
            m2 = hashlib.md5()
            m2.update(codecs.encode(os.path.join(from_dir, 'audio_and_txt_files', root, filename)))
            ...
            if new_filename not in filename_set:
                ...
                rep_index += 1
                filename_set.add(new_filename)
            shutil.copy(os.path.join(from_dir, 'audio_and_txt_files', root, filename),
                        os.path.join(to_dir, 'audio_and_txt_files', new_filename))
            wavs.append('audio_and_txt_files/' + new_filename)
            if root in name_set:
                labels.append(root)
    ...
    data = data.T
    df = pd.DataFrame(data, columns=['audio_and_txt_files_path','patient_id','age',
                                     'sex','adult_bmi','child_weight','child_height','diagnosis'])
