...
class Processor(Base):
    ...
    def input_x(self, audio_and_txt_files_path,patient_id,age,sex,adult_bmi,child_weight,child_height):
        ...
        if not os.path.exists(DATA_PATH + '/' + path):
            ...
            for wavs in wav_path:
                ...
                if re.match('.*.wav', wavs) != None:
                    data, samplerate = soundfile.read(DATA_PATH + '/' + path + '/' + wavs)
                    soundfile.write(DATA_PATH + '/' + path + '/' + wavs, data, samplerate, subtype='PCM_16')
                    sounds.append(AudioSegment.from_wav(DATA_PATH + '/' + path + '/' + wavs))
                    wav += wavs
                    wav_list.append(wav)
            ...
            file_handle = playlist.export(DATA_PATH + '/' + wav, format="wav")
        else:
            wav_path = os.listdir(DATA_PATH + '/' + path)
            wav_list = []
            wav = path + '/'
        ...
        # cbn.build(wav_list, wav_list[0], 'concatenate')
    ...
    def input_y(self, diagnosis):
        ...
        if diagnosis in self.label_dict:
            y[self.label_dict[diagnosis]] = 1
        else:
            y[1] = 1
        return y
