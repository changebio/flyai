...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(100 * 64, 128)  ##  (max sentence length * hidden layer, 512)
...
class LSTMNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.LSTM_stack = nn.LSTM(num_dims, 64, num_layers=1, batch_first=True)
        for name, param in self.LSTM_stack.named_parameters():
            if 'bias' in name:
                nn.init.constant_(param, 0.0)
            elif 'weight' in name:
        ...
        self.relu1 = nn.ReLU(True)
        self.fc1 = nn.Linear(20 * 64 * 2, 128)  ##  (max sentence length * hidden layer, 512)
        self.relu2 = nn.ReLU(True)
        self.dp = nn.Dropout(0.2)
        self.fc2 = nn.Linear(128, 2)
