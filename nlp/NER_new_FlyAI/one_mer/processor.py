...
def predict_data(question):
    ...
    sv = []
    for s in source:
        if s in new_ch_vecs.keys():
            sv.append(new_ch_vecs[s].reshape(2,100))
        else:
            sv.append(np.random.rand(2,100))
    return np.stack(sv)
...
class NLPFlyAI(Dataset):
    def __init__(self, root, df, transform=None):
        ...
        self.w2l = pd.DataFrame(list(self.df2dict().items()),columns=['word','label'])
    ...
    def __getitem__(self, index):
        ...
        return self.w2v[question].reshape((2,100)),answer
