...
class Model(Base):
    ...
    def predict(self, **data):
        ...
        x_1 = torch.from_numpy(x_data)
        #x_2 = torch.from_numpy(x_data[1])
        #x_3 = torch.from_numpy(x_data[2])
        x_1 = x_1.float().to(device)
        print(x_1.shape)
        print(x_1)
        x_1 = x_1.transpose(1,0)
    ...
    def predict_all(self, datas):
        ...
        for data in datas:
            ...
            #x_2 = torch.from_numpy(x_data[1])
            #x_3 = torch.from_numpy(x_data[2])
            x_1 = x_1.transpose(1,0).to(device)
            #x_2 = x_2.float().to(device)
            #x_3 = x_3.float().to(device)
            outputs = network(x_1)
            ...
            print(prediction)
            prediction = self.data.to_categorys(prediction[0])
