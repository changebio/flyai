...
class Processor(Base):
    ...
    def input_x(self, text):
        ...
        if len(tokenized_review) > self.max_sts_len:
            tokenized_review = tokenized_review[:self.max_sts_len]
        ids_review  = tokenizer.convert_tokens_to_ids(tokenized_review)
        padding = [0] * (self.max_sts_len - len(ids_review))
        ids_review += padding
        assert len(ids_review) == self.max_sts_len
        #print(ids_review)
        vec = np.array(ids_review)#.reshape(self.max_sts_len,1)
        return vec
