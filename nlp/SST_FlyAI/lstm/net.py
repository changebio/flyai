...
class GRUNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.LSTM_stack = nn.GRU(num_dims, 64, num_layers=3, batch_first=True)
        for name, param in self.LSTM_stack.named_parameters():
            if 'bias' in name:
                nn.init.constant_(param, 0.0)
            elif 'weight' in name:
        ...
        self.relu1 = nn.ReLU(True)
        self.fc1 = nn.Linear(30 * 64, 64)  ##  (max sentence length * hidden layer, 512)
        self.relu2 = nn.ReLU(True)
        self.dp = nn.Dropout(0.2)
...
class LSTMNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.relu1 = nn.ReLU(True)
        self.fc1 = nn.Linear(20 * 64, 128)  ##  (max sentence length * hidden layer, 512)
        self.relu2 = nn.ReLU(True)
        self.dp = nn.Dropout(0.2)
        self.fc2 = nn.Linear(128, 2)
        #self.bn = nn.BatchNorm1d()
