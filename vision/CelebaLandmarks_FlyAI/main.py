...
def data_describe(data):
    data_summary = data.db.source.data.describe()
    for k in range(data_summary.shape[1]):
        print(list(data_summary.iloc[:,k]))
    for i in range(1,data.db.source.data.shape[1]):
        print(data.db.source.data.iloc[:,i].value_counts())
...
if __name__ == "__main__":
    ...
    val_dataset = DatasetFlyAI(root=DATA_PATH,df=data.db.source.val,transform=val_transforms)
    ...
    net = VGG(settings.net,settings.nc)
