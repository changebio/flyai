...
class VggNetModel(object):
    ...
    def loss(self, batch_x, batch_y=None):
        y_predict = self.inference(batch_x, training=True)
        self.loss = tf.sqrt(tf.reduce_mean(tf.square(batch_y - y_predict))) # rmse
        return self.loss

    def optimize(self, learning_rate, global_step, train_layers=[]):
        var_list = [v for v in tf.trainable_variables() if v.name.split('/')[0] in train_layers]
        return tf.train.AdamOptimizer(learning_rate).minimize(self.loss, var_list=var_list, global_step=global_step)

    def load_original_weights(self, session, skip_layers=[], pretrain_model='vgg16_weights.npz'):
        weights = np.load(pretrain_model)
