...
class Model(Base):
    ...
    def predict_all(self, datas):
        ...
        if cuda_avail:
            cnn.cuda()
        labels = []
        print("****** test data in predict all*******",len(datas),datas[0])
        for i in range(0,len(datas),64):
            x_data = numpy.array([self.data.predict_data(**datas[j])[0] for j in range(i,min(i+64,len(datas)))])
            x_data = torch.from_numpy(x_data)
            #print(x_data.shape)
            if cuda_avail:
                x_data = Variable(x_data.cuda())
